...
type Props = DirectorBlockInterProps & {
  lang?: 'en' | 'ar' | 'fr'
}

export const DirectorBlock: React.FC<Props> = (props) => {
  const { heading, highlight, description, image, link, lang } = props
  ...
  const resolvedHref =
    link?.type === 'custom'
      ? link?.url || '#'
      : typeof link?.reference?.value === 'object' && 'slug' in link.reference.value
        ? `/${lang || 'en'}/${link.reference.value.slug}`
        : '#'
  ...
  return (
    <section
      className={`relative bg-gradient-to-br from-slate-50 via-white to-slate-100 py-16 overflow-hidden ${
        isRTL ? 'rtl' : 'ltr'
      }`}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      <div
        className={`max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-12 items-center ${
          isRTL ? 'md:grid-flow-col-dense' : ''
        }`}
      >
        {/* Text content */}
        <div
          className={`${isRTL ? 'text-right md:order-2' : 'text-left'} text-center md:text-left`}
        >
          <h2
            className={`text-4xl font-bold text-slate-800 font-playfair-display mb-4 leading-snug ${
              isRTL ? 'text-right' : 'text-left'
            }`}
          >
            {heading} {highlight && <span className="text-[#D78B22]">{highlight}</span>}.
          </h2>
          <p className={`text-lg text-slate-600 mb-8 ${isRTL ? 'text-right' : 'text-left'}`}>
            {description}
          </p>
          {link?.label && resolvedHref && (
            <Link
        ...
        {/* Image */}
        <div
          className={`relative w-full h-[280px] md:h-[340px] max-w-md rounded-3xl overflow-hidden shadow-xl border-4 border-white group ${
            isRTL ? 'md:order-1' : ''
          }`}
        >
          <div
