...
// interface WhoWeAreTabsProps extends WhoweareInterProps {
//   lang?: 'ar' | 'en' | 'fr'
// }

// export const WhoWeAreTabs: React.FC<WhoWeAreTabsProps> = ({ lang, ...props }) => {
//   const [activeTab, setActiveTab] = useState(0)
//   const isRTL = lang === 'ar'
...
//                           </div>
//                           <div
//                             className={`absolute top-4 ${isRTL ? 'left-4' : 'right-4'} bg-gradient-to-r from-[#D78B22] to-[#f4a335] text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg`}
//                           >
//                             {String(index + 1).padStart(2, '0')}
//                           </div>
//                         </div>
                ...
                {props.link?.label && (props.link.url || props.link.reference) && (
                  <a
                    href={
                      ...
                      (typeof props.link.reference?.value === 'object'
                        ? 'slug' in props.link.reference.value
                          ? `/${lang || 'en'}/${props.link.reference.value.slug}`
                          : '#'
                        : '#')
                    }
                    target={props.link.newTab ? '_blank' : '_self'}
                  ...
                  >
                    {isRTL && (
                      <ArrowRight className="w-4 h-4 mr-2 rotate-180 transform group-hover:-translate-x-1 transition-transform duration-300" />
                    )}
                    {props.link.label}
                    {!isRTL && (
                      <ArrowRight className="w-4 h-4 ml-2 transform group-hover:translate-x-1 transition-transform duration-300" />
