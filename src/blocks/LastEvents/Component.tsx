...
interface LastEventBlockProps {
  header: string
  lang: 'en' | 'ar' | 'fr'
}

const queryEventBySlug = async (lang: 'en' | 'ar' | 'fr'): Promise<EventType[]> => {
  const { isEnabled: draft } = await draftMode()
  const payload = await getPayload({ config: configPromise })
  ...
  const result = await payload.find({
    ...
    overrideAccess: draft,
    sort: '-createdAt',
    locale: lang, // 👈 Pull localized content
  })

  return result.docs as EventType[]
}

const queryUserByID = async (id: string, lang: 'en' | 'ar' | 'fr'): Promise<UserType | null> => {
  const { isEnabled: draft } = await draftMode()
  const payload = await getPayload({ config: configPromise })
  ...
  const result = await payload.find({
    ...
    pagination: false,
    overrideAccess: draft,
    locale: lang, // 👈 Localized user content if needed
    where: {
      id: { equals: id },
    },
  })
...
export const LastEventBlock = async ({ header, lang }: LastEventBlockProps) => {
  const isRTL = lang === 'ar'
  const posts = await queryEventBySlug(lang)
  ...
  const postsWithAuthors = await Promise.all(
    posts.map(async (post) => {
      ...
      let user: UserType | null = null
      if (author && typeof author === 'object' && 'id' in author) {
        user = await queryUserByID(String(author.id), lang)
      }
      return { post, user }
    }),
  )
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="py-16 md:py-24 lg:py-32">
          ...
          <div className="max-w-4xl mx-auto text-center pb-16 md:pb-20">
            ...
            <div className="w-24 h-1 bg-gradient-to-r from-[#D78B22] to-[#f4a335] mx-auto rounded-full" />
          </div>
          ...
          <div className="max-w-sm mx-auto md:max-w-none grid gap-8 lg:gap-12 md:grid-cols-2 lg:grid-cols-3 items-start">
            {postsWithAuthors.map(({ post }, i) => {
              ...
              return (
                ...
                >
                  <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-6 transition-all duration-500 ease-out hover:bg-white/10 hover:transform hover:scale-105 border border-white/10 hover:border-[#D78B22]/30 shadow-xl hover:shadow-2xl">
                    <Link href={`/${lang}/events/${post.slug}`} className="relative block mb-6">
                      <div className="absolute inset-0 bg-gradient-to-r from-[#D78B22] to-[#f4a335] rounded-2xl opacity-0 group-hover:opacity-20 transition-all duration-300 ease-out transform group-hover:scale-105 blur-xl"></div>
                      <div className="relative overflow-hidden rounded-2xl shadow-lg">
                        {typeof post.heroImage === 'object' && post.heroImage !== null ? (
                          <Media
                        ...
                        <div
                          className={`absolute top-4 ${isRTL ? 'left-4' : 'right-4'} bg-[#D78B22] text-white px-3 py-1 rounded-full text-sm font-semibold shadow-lg`}
                        >
                          {lang === 'ar' ? 'الأحدث' : 'Dernière'}
                        </div>
                      </div>
                    </Link>
                    ...
                    <div className="pt-6">
                      ...
                      <Link
                        href={`/${lang}/events/${post.slug}`}
                        className={`inline-flex items-center mt-4 text-[#D78B22] hover:text-[#f4a335] font-semibold transition-colors duration-300 group/link ${isRTL ? 'flex-row-reverse' : ''}`}
                      >
                        {lang === 'ar' ? 'اقرأ المزيد' : 'En savoir plus'}
                        <svg
                          className={`w-4 h-4 ${isRTL ? 'mr-2 rotate-180' : 'ml-2'} transform group-hover/link:translate-x-1 transition-transform duration-300`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
          ...
          <div className="flex justify-center mt-16 md:mt-20">
            ...
            >
              <span className="relative z-10">
                {lang === 'ar' ? 'عرض جميع الأحداث' : 'Voir tous les événements'}
              </span>
              <svg
                className={`w-5 h-5 ${isRTL ? 'mr-3 rotate-180' : 'ml-3'} transform group-hover:translate-x-1 transition-transform duration-300`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
